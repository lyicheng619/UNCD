...
def unlearn(
    ...
    if some_pt_model_dir is None or some_ft_model_dir is None:
        raise ValueError("Task vector requires pretrained and finetuned models!")

    # Compute Task Vector
    pt_model = load_model(some_pt_model_dir)
    ft_model = load_model(some_ft_model_dir)

    task_vector = TaskVector(
        pretrained_state_dict=pt_model.state_dict(),
        finetuned_state_dict=ft_model.state_dict()
    )
    ...
    # Apply Negative Task Vector
    model = load_model(model_dir)
    new_state_dict = neg_task_vector.apply_to(pretrained_model=model, scaling_coef=alpha, in_place=False)

    print("Task vector applied. State dict updated.")

    # Save Unlearned Model
    #new_model = load_model(model_dir, state_dict=new_state_dict, device_map='cpu')
    new_model = load_model(model_dir)
    new_model.load_state_dict(new_state_dict, strict=False) 
    if out_dir is not None:
        new_model.save_pretrained(out_dir, safe_serialization=True)
        print(f"Unlearned model saved to {out_dir}.")
    return new_model
...
class TaskVector:
    def __init__(self,
        ...
        if vector is not None:
            ...
            with torch.no_grad():
                ...
                # Debugging: print task vector norms
                for key, value in self.vector.items():
                    print(f"Task vector key: {key}, Norm: {torch.norm(value)}")
    ...
    def is_nonzero(self):
        """Check if the task vector is non-zero."""
        return any([(self.vector[key] != 0).any() for key in self.vector])
        ...
        with torch.no_grad():
            ...
            for key in pretrained_state_dict:
                if key not in self.vector:
                    print(f"Warning: key {key} is present in the pretrained state dict but not in the task vector")
                    continue
                diff = scaling_coef * self.vector[key]
                #print(diff)
                print(f"Key: {key}, Task Vector Norm: {torch.norm(self.vector[key])}, Scaling Coef: {scaling_coef}, Diff Norm: {torch.norm(diff)}")
                new_state_dict[key] = pretrained_state_dict[key] + diff
            

            for key in new_state_dict:
                if key in self.vector:
                    is_different = not torch.equal(pretrained_state_dict[key], new_state_dict[key])
                    print(f"Key: {key}, Is Different: {is_different}")
